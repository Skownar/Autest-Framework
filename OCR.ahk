...
{
  this.xywh2xywh(x-w,y-h,2*w+1,2*h+1,x,y,w,h)
  if (w<1 or h<1)
    Return, 0
  bch:=A_BatchLines
  SetBatchLines, -1
  ;--------------------------------------
  this.GetBitsFromScreen(x,y,w,h,Scan0,Stride,bits)
  ;--------------------------------------
  sx:=0, sy:=0, sw:=w, sh:=h
  ...
  Loop, Parse, text, |
  {
    ...
    color:=r1, v:=r2
    StringSplit, r, v, .
    w1:=r1, v:=this.base64tobit(r2), h1:=StrLen(v)//w1
    if (r0<2 or w1>sw or h1>sh or h1<1 or StrLen(v)!=w1*h1)
      Continue
    if this.PicFind(Scan0,Stride,sx,sy,sw,sh
